...
- **Client-shipped `.env` exposes a reusable AI proxy bearer token** — Medium — `pubspec.yaml`. The Hack Club AI proxy token is a personal bearer token, not a shared community key, so if it's bundled into the shipped app/`.env` it's a real, usable credential exposure, not a scanner false positive. This needs a hard-stop manual rotation once removed from the client — never have an agent auto-rotate this.
...
- **Path traversal attack possible** — High — `local_storage_service.dart` and `document_repository.dart`. Needs checking for unsanitized file paths being built from user- or AI-derived input (e.g. document names) before reading/writing to disk.
...
- **Unsafe deserialization can lead to remote code execution** — Medium — flagged against `kotlin-stdlib`. Worth confirming whether this is a real usage pattern in the notch/notification Kotlin code or a transitive dependency false positive.
- **Android components with exported attribute active** — Medium — `AndroidManifest.xml`. Likely overlaps with the intents-not-received-safely finding from SonarQube (see below) and the `BootReceiver` finding above.
...
- **L37** — Obfuscation is not confirmed enabled in the release build config. High severity, Security.
...
- **L85** — Intents may not be received safely (exported component without proper guarding). High/Critical, Security. This is almost certainly the same surface as the Aikido "exported attribute" and `BootReceiver` findings above.

### `NekoNotificationListenerService.kt`
- Empty/unfilled code blocks at **L64, L79, L90, L384, L446** — either dead placeholder code left in, or logic that was started and never finished.
...
### `NotchBridge.kt`
- Empty/unfilled code blocks at **L84, L120, L133, L145, L157, L202, L212, L222**.
