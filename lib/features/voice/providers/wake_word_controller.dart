...
class WakeWordController extends Notifier<WakeWordState> {
  ...
  void _onLifecycle(AppLifecycleState s) {
    ...
    if (_foreground && state.enabled && !_paused) {
      ...
      // case the system reclaimed it, and kick the loop if the recogniser died.
      unawaited(_startService());
      unawaited(_startLoop());
    }
  ...
  Future<void> _startLoop() async {
    if (!state.enabled || _paused || state.listening) return;
    final bool ready = await _speech.ensureReady(
      onStatus: _onStatus,
      onError: _onError,
    );
    if (!ready || !state.enabled || _paused) return;
